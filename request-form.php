
<div class="trh-container">
    <div class="trh-request-form">
        <form class="form" method="post" action="" id="trh-request-form">
            <div class="trh-field-row trh-field-row-split">
                <div class="trh-field-column">
                    <label for="trhrf_start_date"><?=__('Pick-up date', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                    <input type="text" class="trh-input-control" name="trhrf_start_date" id="trhrf_start_date" placeholder="<?=__('Select date', 'therentalshub-request');?>..." required/>
                </div>
                <div class="trh-field-column">
                    <label for="trhrf_start_time"><?=__('Pick-up time', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                    <input type="text" class="trh-input-control" name="trhrf_start_time" id="trhrf_start_time" placeholder="<?=__('Select time', 'therentalshub-request');?>..." required/>
                </div>
            </div>

            <div class="trh-field-row trh-field-row-split">
                <div class="trh-field-column">
                    <label for="trhrf_end_date"><?=__('Drop-off date', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                    <input type="text" class="trh-input-control" name="trhrf_end_date" id="trhrf_end_date" placeholder="<?=__('Select date', 'therentalshub-request');?>..." required/>
                </div>
                <div class="trh-field-column">
                    <label for="trhrf_end_time"><?=__('Drop-off time', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                    <input type="text" class="trh-input-control" name="trhrf_end_time" id="trhrf_end_time" placeholder="<?=__('Select time', 'therentalshub-request');?>..." required/>
                </div>
            </div>

            <?php if ($trhShowCars === 'true') : ?>
            <div class="trh-field-row">
                <label for="trhrf_car"><?=__('Choose vehicle', 'therentalshub-request');?></label>
                <select class="trh-input-control" name="trhrf_car" id="trhrf_car">
                    <option value="0"></option>
                    <?php foreach ($cars as $group) : ?>
                        <optgroup label="<?=sprintf(__('Group: %s', 'therentalshub-request'), esc_attr($group->category));?>">
                            <?php foreach ($group->cars as $car) : ?>
                                <option value="<?=esc_attr($car->id);?>" <?php selected($trhSelectedCarId, $car->id); ?>><?=esc_html($car->name);?></option>
                            <?php endforeach; ?>
                        </optgroup>
                    <?php endforeach; ?>
                </select>
            </div>
            <?php endif; ?>

            <?php if ($trhShowLocations === 'true') : ?>
            <div>
                <div class="trh-field-row">
                    <label for="trhrf_pick_loc"><?=__('Choose pick-up location', 'therentalshub-request');?></label>
                    <select class="trh-input-control" name="trhrf_pick_loc" id="trhrf_pick_loc">
                        <option value="0"></option>
                        <?php foreach ($locations as $loc) : ?>
                            <option value="<?=esc_attr($loc->id);?>"><?=esc_html($loc->name);?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="trh-field-row">
                    <label for="trhrf_drop_loc"><?=__('Choose drop-off location', 'therentalshub-request');?></label>
                    <select class="trh-input-control" name="trhrf_drop_loc" id="trhrf_drop_loc">
                        <option value="0"></option>
                        <?php foreach ($locations as $loc) : ?>
                            <option value="<?=esc_attr($loc->id);?>"><?=esc_html($loc->name);?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <?php endif; ?>

            <?php if ($trhShowFlightNr === 'true') : ?>
            <div class="trh-field-row">
                <label for="trhrf_flight"><?=__('Flight number or ship name', 'therentalshub-request');?></label>
                <input type="text" class="trh-input-control" name="trhrf_flight" id="trhrf_flight" placeholder="<?=__('Flight number or ship name', 'therentalshub-request');?>"/>
            </div>
            <?php endif; ?>

            <div class="trh-field-row">
                <label for="trhrf_fname"><?=__('First name', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                <input type="text" class="trh-input-control" name="trhrf_fname" id="trhrf_fname" placeholder="<?=__('First name', 'therentalshub-request');?>" required/>
            </div>

            <div class="trh-field-row">
                <label for="trhrf_lname"><?=__('Last name', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                <input type="text" class="trh-input-control" name="trhrf_lname" id="trhrf_lname" placeholder="<?=__('Last name', 'therentalshub-request');?>" required/>
            </div>

            <div class="trh-field-row">
                <label for="trhrf_email"><?=__('Email address', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                <input type="text" class="trh-input-control" name="trhrf_email" id="trhrf_email" placeholder="<?=__('Email address', 'therentalshub-request');?>" required/>
            </div>

            <div class="trh-field-row">
                <label for="trhrf_phone"><?=__('Phone number', 'therentalshub-request');?> <span class="trh-required">*</span></label>
                <input type="text" class="trh-input-control" name="trhrf_phone" id="trhrf_phone" placeholder="<?=__('Phone number', 'therentalshub-request');?>" required/>
            </div>

            <div class="trh-field-row">
                <label for="trhrf_notes"><?=__('Additional notes', 'therentalshub-request');?></label>
                <textarea class="trh-input-control" rows="5" name="trhrf_notes" id="trhrf_notes" placeholder="<?=__('Additional notes', 'therentalshub-request');?>"></textarea>
            </div>

            <div id="trh-request-failed-alert" role="alert" style="display:none">
                <div class="trh-alert trh-error-alert"></div>
            </div>

            <div id="trh-request-success-alert" role="alert" style="display:none">
                <div class="trh-alert trh-success-alert">
                    <?=__('Thank you! Your request has been submitted. We will contact you shortly with availability. Please check your inbox for our confirmation email.', 'therentalshub-request');?>
                </div>
            </div>

            <div class="trh-field-row">
                <input type="hidden" name="trhrf_car_name" id="trhrf_car_name" value="">
                <input type="hidden" name="trhrf_pick_loc_name" id="trhrf_pick_loc_name" value="">
                <input type="hidden" name="trhrf_drop_loc_name" id="trhrf_drop_loc_name" value="">
                <button type="submit" class="trh-button"><?=__('Submit booking request', 'therentalshub-request');?></button>
            </div>
        </form>

        <script>
            var trhApp = {
                minDays: <?=$trhMinDays;?>,
                defaultTime: "<?=$trhDefaultTime;?>",
                showCars: <?=$trhShowCars;?>,
                carsByGroup: <?=$trhCarsByGroup;?>,
                showLocations: <?=$trhShowLocations;?>,
                showFlightNr: <?=$trhShowFlightNr;?>,
                lang: {
                    warn_fill_fields: "<?=__('Please fill all required (with the asterisk) fields.', 'therentalshub-request');?>",
                    please_wait: "<?=__('Please wait', 'therentalshub-request');?>"
                }
            };
        </script>

    </div>
</div>